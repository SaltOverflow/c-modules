...
int main(int argc, char *argv[]) {
    ...
    for (int i=0; i<args.num_iter; ++i) {
        struct graph$$Graph$$stub g = graph$$create_rand_graph(args.num_nodes, args.num_edges);
        int paths_found = 0;
        for (int j=0; j<args.num_checks; ++j) {
            struct graph$node$$Node$$stub *n1 = graph$$grab_random_node(&g);
            struct graph$node$$Node$$stub *n2 = graph$$grab_random_node(&g);
