...
class ListenerTopLevel(CListener):
    def __init__(self):
        ...
        # output
        self.functions = {}  # dict[name:str, ctx:functionDefinitionContext]
        self.structs = {}  # dict[name:str, ctx:structSpecifierContext]
        self.variables = []  # list[(list[name:str], ctx:declarationContext)], only contains top-level
    ...
    def exitFunctionDefinition(self, ctx):
        ...
        if self.outer:
            ...
            return
        name = self._extract_name_from_declarator(ctx.declarator())
    ...
    def exitStructSpecifier(self, ctx):
        ...
        if name in self.structs:
            ...
            self.structs[name] = ctx
    
    def enterDeclaration(self, ctx):
        if not ctx.initDeclaratorList() and not self._extract_name_from_declarationSpecifiers(ctx.declarationSpecifiers()):
            return
        self.outer.append(ctx)
    
    def exitDeclaration(self, ctx):
        if not ctx.initDeclaratorList():
            name = self._extract_name_from_declarationSpecifiers(ctx.declarationSpecifiers())
            if not name:
                return
        should_be_ctx = self.outer.pop()
        assert(should_be_ctx is ctx)
        ...
        if not ctx.initDeclaratorList():
            ...
            names = [self._extract_name_from_declarator(x.declarator()) for x in ctx.initDeclaratorList().initDeclarator()]
        ...
        if colliding_names:
            ...
            self.variables.append((names, ctx))

    def _extract_name_from_declarator(self, ctx:CParser.DeclaratorContext) -> str:
        ...
        if directDeclarator.declarator():
            return self._extract_name_from_declarator(directDeclarator.declarator())
        return directDeclarator.Identifier().getText()

    def _extract_name_from_declarationSpecifiers(self, ctx:CParser.DeclarationSpecifiersContext) -> str | None:
        ...
        for declarationSpecifier in ctx.declarationSpecifier():
            ...
            if not typeSpecifierParsed:
                ...
                if typedefName:
                    return typedefName.getText()  # stuff after may be invalid, but I don't check it
        return None
